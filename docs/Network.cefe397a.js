...
function () {
  ...
  _createClass(Network, [{
    ...
    value: function drawNetworkGraph(word, since, until, politicians, sentiments, period) {
      var _this = this;
      ...
      d3.json(graphfile).then(function (data) {
        d3.json(tweetsfile).then(function (tweetData) {
          ...
          _this.drawNetwork(nodes, links, linkId);
        });
      });
    }
  }, {
    key: "drawNetwork",
    value: function drawNetwork(nodes, links, linkId) {
      ...
      function drawLink(d) {
        context.moveTo(d.source.x, d.source.y); // context.lineTo(d.target.x, d.target.y);
